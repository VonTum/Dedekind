...
void pawelskiAllIntervalsToTop() {
	...
	const Monotonic<Variables>* allBigEqClasses = readFlatBuffer<Monotonic<Variables>>(FileName::flatMBFs(Variables), mbfCounts[Variables]);
	
	const FlatNode* allDuals = readFlatBuffer<FlatNode>(FileName::flatNodes(Variables), mbfCounts[Variables]);
	
	const ClassInfo* allBigIntervalSizes = readFlatBuffer<ClassInfo>(FileName::flatClassInfo(Variables), mbfCounts[Variables]);
	
	PawelskiPartialIntervalCounter<Variables> partialIntervalCounter;

	for(size_t i = 0; i < dedekindNumbers[Variables-2] * dedekindNumbers[Variables-2]; i++) {
		if(smallIntervalLUT[i] == 0xFFFF) continue;
		uint16_t sum = 0;
		for(size_t j = 0; j <= (1 << (Variables-2)); j++) {
			sum += partialIntervalCounter.smallerIntervalSizeMatrix3D[dedekindNumbers[Variables-2] * dedekindNumbers[Variables-2] * j + i];
		}
		if(sum != smallIntervalLUT[i]) {
			std::cout << "Invalid new interval size: " << sum << " != " << smallIntervalLUT[i] << std::endl;
		}
	}
	...
	iterRangeInParallelBlocksOnAllCores<size_t>(0, mbfCounts[Variables], 128, [&](int coreI, size_t i) {
		if(i % 2048 == 0) std::cout << std::to_string(i) + "\n" << std::flush;
		Monotonic<Variables> bot = allBigEqClasses[i];
		uint64_t intervalSize = pawelskiIntervalToTopSize(bot, smallMBFLUT.get(), inverseIndexMap.get(), aboveLUT.get(), smallIntervalLUT.get());
	...
	freeFlatBuffer<Monotonic<Variables>>(allBigEqClasses, mbfCounts[Variables]);
	freeFlatBuffer<FlatNode>(allDuals, mbfCounts[Variables]);
	freeFlatBuffer<ClassInfo>(allBigIntervalSizes, mbfCounts[Variables]);
}
